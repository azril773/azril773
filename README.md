# Mohammad Azril Sugiarto

> Backend Engineer | System Optimization | Scalable Solutions


## About

Backend-focused Software Engineer with 5 years of proven expertise in building optimized systems and robust APIs. Specializes in delivering measurable performance improvements and production-ready infrastructure.


## Highlights

**83%** improvement in attendance processing efficiency  
**Legacy system migrations** with complex edge case handling  
**Production infrastructure** setup using Docker & Nginx  
**Full-stack development** across multiple client projects


## Tech Stack

**Backend:** NestJS · Node.js · Go · PHP · Python  
**Database:** PostgreSQL · MySQL · MongoDB · Redis  
**DevOps:** Docker · Nginx · Git


## Experience

**PT Asia Tritunggal Jaya** — Software Engineer *(June 2024 – Present)*  
Optimized system performance, executed legacy migrations, and developed inventory management solutions.

**PT Cirebon Digital Solution** — Freelance Web Developer *(Dec 2024 – Oct 2025)*  
Delivered multiple client projects focused on data efficiency and user experience.

**PT Fath Technology Solution** — Software Engineer Intern *(Dec 2023 – Apr 2024)*  
Full-stack development with NestJS & ReactJS using semi-monolithic architecture.


## Contact

azrilsugiarto06@gmail.com · +62 8953 8320 8655 · Cirebon, Indonesia · [Azril Sugiarto](https://www.linkedin.com/in/azrilsugiarto/)
